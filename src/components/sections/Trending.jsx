const Trending = ({name, year, category, rating, photo, bookmark, onMarcar, valor}) => {
  return (
    <div className="relative cursor-pointer h-[200px] group/principal">
        {
            bookmark === true ? (
                <div onClick={() => onMarcar(valor)} className="absolute w-10 h-10 2xl:ml-[440px] xl:ml-[345px] md:ml-80 md:mt-4 mt-2 ml-[330px] hover:bg-white hover:bg-opacity-100 hover:z-30 bg-opacity-50 flex items-center justify-center rounded-full bg-black z-10 group/bookmark">
        ...
        <div className="absolute hidden group-hover/principal:flex items-center bg-white bg-opacity-50 px-2 py-[5px] rounded-3xl 2xl:ml-[215px] xl:ml-36 2xl:mt-[100px] xl:mt-20 md:ml-36 md:mt-16 ml-36 mt-16">
            ...
            <div className="ml-3 pr-2 relative text-white group-hover/principal:opacity-100 z-10">Play</div>
        </div>
        <img className="rounded-md group-hover/principal:opacity-50" src={photo} alt="photo-trending" />
        <div className="-mt-14 ml-3">
            <div className="text-white opacity-80 flex gap-x-1 font-light">
                ...
                <div className="2xl:flex md:flex md:items-center hidden">•</div>
                <div className="md:relative absolute md:bg-transparent md:px-0 md:py-0 md:rounded md:bg-opacity-0 md:ml-0 md:mt-0 bg-white px-2 py-[2px] rounded-2xl bg-opacity-20 ml-[320px] mt-2">{rating}</div>
            </div>
            <div className="relative text-white text-[18px] group-hover/principal:opacity-100">{name}</div>
        </div>
    </div>
  )
}
